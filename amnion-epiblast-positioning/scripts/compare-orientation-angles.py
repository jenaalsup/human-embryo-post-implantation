#!/usr/bin/env python3
"""
Compare orientation angles between amnion, intermediate, and epiblast nuclei.
Box plot + scatter.
"""
...
for csv_file in INPUT_FILES:
    ...
    for n in nuclei:
        ...
        if n['type'] == 'A':
            ...
            intermediate_angles.append(orientation)
        elif n['type'] == 'E':
            epiblast_angles.append(orientation)
    
    a_count = len([n for n in nuclei if n['type'] == 'A'])
    i_count = len([n for n in nuclei if n['type'] == 'I'])
    e_count = len([n for n in nuclei if n['type'] == 'E'])
    print(f"✓ {os.path.basename(csv_file)}: {a_count} amnion, {i_count} intermediate, {e_count} epiblast")

# Statistical test (Welch's t-test) - Amnion vs Epiblast only
def welch_ttest(x, y):
...
print(f"Epiblast: n={len(epiblast_angles)}, mean={sum(epiblast_angles)/len(epiblast_angles):.1f}°")
print(f"\nWelch's t-test (Amnion vs Epiblast): t={t_stat:.3f}, df={df:.1f}, p={pvalue:.4f}")
...
# Prepare data
data = [amnion_angles, intermediate_angles, epiblast_angles]
labels = ['Amnion', 'Intermediate', 'Epiblast']
colors = ['#1E88E5', '#9C27B0', '#FF8C00']
...
# Add significance annotation (Amnion vs Epiblast)
y_max = max(max(amnion_angles), max(intermediate_angles), max(epiblast_angles))
y_pos = y_max + 5

ax.plot([1, 1, 3, 3], [y_pos, y_pos+2, y_pos+2, y_pos], 'k-', linewidth=1.5)
ax.text(2, y_pos+3, sig, ha='center', va='bottom', fontsize=14, fontweight='bold')
ax.text(2, y_pos+6, f'p={pvalue:.4f}', ha='center', va='bottom', fontsize=10)
...
# Add sample sizes
for i, (label, n) in enumerate(zip(labels, [len(amnion_angles), len(intermediate_angles), len(epiblast_angles)])):
...
print(f"\n✓ Plot saved: {OUTPUT}")
print(f"✓ Plot saved: {OUTPUT.replace('.png', '.eps')}")
...
print(f"✓ Raw data saved: {os.path.basename(output_csv)}")
